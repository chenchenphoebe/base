...
#             if self._device(resourceId='com.android.email:id/dotdotdot').exists:
            ...
            self._device(description ='Show accounts').wait.exists(timeout=5000)
    ...
    #             if self._device(resourceId='com.google.android.gm:id/name',text=box).exists:
    ...
    def forward_email(self, index, address):
        ...
        if not self.select_mail(index):
            return False   
        if self._device(resourceId = 'com.google.android.gm:id/title_container', instance = 1).exists:
            self._device(resourceId = 'com.google.android.gm:id/title_container', instance = 0).click()
            self._device.delay(2) 
        ...
        self._device(resourceId='com.google.android.gm:id/subject').clear_text()
        self._device.delay(2)
        try_set_title_times = 0
        while not self._device(resourceId='com.google.android.gm:id/subject').get_text().find('Forward') > -1:
            self._logger.debug('Input titile.') 
            self._device(resourceId='com.google.android.gm:id/subject').set_text('Forward_' + datetime.now().strftime('%Y-%m-%d-%H-%M-%S'))
            self._device.press.enter()
            self._device.delay(2)
            try_set_title_times += 1
            if try_set_receiver_times > 3:
                self._logger.debug('Input title fail!')
                return False
    ...
    def select_mail(self,index):
        ...
        self._device.delay(2)
        if (self._device( descriptionContains ='Delete').wait.exists(timeout=10000))\
        and (not self._device(resourceId ='com.google.android.gm:id/loading_progress').wait.exists(timeout=10000)):
            return True
        else:
