...
class Alarm(Common):
    ...
    def stay_in_alarm(self):
        ...
        if self.get_current_packagename() == self.get_app_package_from_file('Clock'):
            while not self._device(className='android.support.v7.app.ActionBar$Tab').exists:
    ...
    def discheck_alarm_current_page(self):
        ...
        """
        if self._device(resourceId='com.tct.timetool:id/onoff').exists:
            for i in range(self._device(resourceId='com.tct.timetool:id/onoff').count):
                if self._device(resourceId='com.tct.timetool:id/onoff', instance=i).isChecked():
                    self._logger.debug('Disable one alarm.')
                    self._device(resourceId='com.tct.timetool:id/onoff', instance=i).click()
                    self._device.delay(1)
        if not self._device(resourceId='com.tct.timetool:id/onoff', checked=True).exists:
            return True
        else:
    ...
    def delete_alarm(self):
        ...
        is_last = False
        if self._device(resourceId='com.android.deskclock:id/alarms_recycler_view').exists \
                and self._device(resourceId='com.android.deskclock:id/arrow').exists:

            if self._device(resourceId='com.android.deskclock:id/alarms_recycler_view').isScrollable():
                self._device(resourceId='com.android.deskclock:id/alarms_recycler_view',
                             scrollable=True).scroll.vert.toEnd()
            if self._device(resourceId='com.android.deskclock:id/arrow', instance=0).exists:
            ...
            if self._device(resourceId='com.android.deskclock:id/delete').exists:
                ...
                if self._device(resourceId='com.android.deskclock:id/snackbar_text',
                                textContains='Alarm deleted').exists:
