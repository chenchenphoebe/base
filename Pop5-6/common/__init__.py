...
#鍏冪粍()鍙傛暟锛歞ef function(*ARG)  
#瀛楀吀{}鍙傛暟锛歞ef function(**ARG)
...
class JsonRPCMethod(object):
    ...
    def id(self):
        ...
        return m.hexdigest()##鍔犲瘑鍚庣殑缁撴灉锛岀敤鍗佸叚杩涘埗瀛楃涓茶〃绀恒��
...
###############################################################################################
    ...
    def getMatchedCenterOffset(self,subPath, srcPath, threshold=0.03, rotation=0):
        ...
        try:
            ...
            result = cv2.matchTemplate(subImg, srcImg, method) #comparision, 瓒婂皬鐨勬暟鍊间唬琛ㄦ洿楂樼殑鍖归厤缁撴灉
            minVal, maxVal, minLoc, maxLoc = cv2.minMaxLoc(result) #Get the minimum squared difference,minMaxLoc瀵绘壘鐭╅樀(涓�缁存暟缁勫綋浣滃悜閲�,鐢∕at瀹氫箟) 涓渶灏忓�煎拰鏈�澶у�肩殑浣嶇疆. 
