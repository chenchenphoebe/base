...
class Camera(Common):
    ...
    def get_current_mode(self):
        ...
        while True:
            ...
            if wait_time > 3:
                self._logger.debug("Can't get the three_dots button.")
                return mode_current
            
        if self._device(resourceId = 'com.android.camera2:id/mode_options_buttons').exists:
            if self._device(resourceId = 'com.android.camera2:id/mode_options_buttons').getChildCount() == 2:
    ...
    def Switch_mode(self,mode):
        ...
        if mode[0] <> mode_current[0] and mode[0] <> '':
            ...
            while not self._device(resourceId='com.android.camera2:id/selector_text').exists:
                if not self._device(resourceId='com.android.camera2:id/mode_options_overlay').exists:
                    self._logger.debug('Swipe view is not exist.')
                    return False
...
#                 self._device(resourceId = 'com.android.camera2:id/filmstrip_view').swipe.left(steps=3)
                self._device.swipe(650, 610, 300, 610)
                self._device.delay(2)
...
#                 self._device(text='Just once').click()
        ...
        if self._device(resourceId ='com.android.gallery3d:id/surface_view').exists\
            ...
            if not self._device(resourceId = 'com.android.camera2:id/filmstrip_view').wait.exists(timeout=30000):
                self._device.press.back()
            if self._device(resourceId = 'com.android.camera2:id/filmstrip_view').exists:
                self._device.delay(2)
    ...
    def delete_media_file(self, preview_type, file_type):
        ...
        while True:
            ...
            if self._device(description='Delete').exists:
                ...
                self._device.delay(3)
                if self._device(resourceId = 'com.android.camera2:id/camera_undo_deletion_bar').wait.exists(timeout=10000):
                    break
                elif not self._device(resourceId = 'com.android.camera2:id/camera_undo_deletion_bar').exists\
        ...
        while self._device(resourceId = 'com.android.camera2:id/camera_undo_deletion_bar').wait.exists(timeout=10000):
            self._logger.debug("Confirm delete.")
            if self._device(resourceId = 'com.android.camera2:id/camera_deletion_button').exists:
                self._device(resourceId = 'com.android.camera2:id/camera_deletion_button').click()
            elif self._device(text='Deleted').exists:
        ...
        if not self._device(resourceId = 'com.android.camera2:id/camera_undo_deletion_bar').exists\
        and self.get_file_num(StorePath_Media, file_type) == file_number-1:
            self._logger.warning("Delete %s successfully!", preview_type)
            return True
        else:
