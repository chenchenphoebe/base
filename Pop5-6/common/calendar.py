...
class Calendar(Common):
    ...
    def stay_in_calendar(self):
        ...
        """
        maxtime = 0
        while not  self._device(description ='Today', resourceId = 'com.android.calendar:id/action_today').exists:
    ...
    def _check_view_type(self, strsort):
        ...
        """
        if (strsort == "Day" and self._device(text="Day").exists):
            self._logger.debug("Current view is Day")
            return True
        elif (strsort == "Week" and self._device(text="Week").exists):
            self._logger.debug("Current view is Week")
            return True
        elif (strsort == "Month" and self._device(text="Month").exists):
            self._logger.debug("Current view is Month")
            return True
        elif (strsort == "Agenda" and self._device(text="Agenda").exists):
            self._logger.debug("Current view is Agenda")
            return True
        elif (strsort == "Year" and self._device(text="Year").exists):
...
#         self._device.delay(2)     
    ...
    def calendar_is_added(self, strname):
        ...
        if self._device(resourceId= 'com.android.calendar:id/agenda_events_list').\
        child_by_text(strname, resourceId = 'com.android.calendar:id/title', allow_scroll_search=True,).exists:
            return True
        else:
    ...
    def create_schedule(self, event_name, isSetDate, index = 1):
        ...
        if self._device(resourceId ='com.android.calendar:id/action_create_event').exists:
            self._device(resourceId ='com.android.calendar:id/action_create_event').click()
        else:
        ...
        self._device(text='Event name').set_text(event_name)
        self._device.delay(2)
        if isSetDate:
            self.set_start_date(index) 
        self._device(resourceId ='com.android.calendar:id/action_done').click()
        self._device.delay(2)     
    ...
    def set_start_date(self, index):
        ...
        """
        if not self._device(resourceId ='com.android.calendar:id/start_date').exists:
            self._logger.debug('The setting date isn\'t exist.' )
            return False
        date_begin_text = self._device(resourceId ='com.android.calendar:id/start_date').get_text()
        date_begin, type = self.format_date(date_begin_text)
        self._device(resourceId ='com.android.calendar:id/start_date').click()        
        self._device.delay(2)
        if self._device(resourceId ='com.android.calendar:id/date_picker_header').exists:
# Set the day to the next day. The from day will change to the set day when open the setting page next time after setting this time.
            ...
            self._device.delay(2)
            self._device(resourceId ='com.android.calendar:id/done').click()
            self._device.delay(2)
        date_text = self._device(resourceId ='com.android.calendar:id/start_date').get_text()
    ...
    def set_date(self, index, current_date):
        ...
        for i in range(drag_times):
            self._device.drag(313, 832, 313, 484)
            self._device.delay(2)
    ...
    def delete_calendar(self):
        ...
        is_last = False
        if self._device(resourceId= 'com.android.calendar:id/agenda_events_list').exists \
            and self._device(resourceId= 'com.android.calendar:id/agenda_item_text_container').exists:
            if self._device(resourceId= 'com.android.calendar:id/agenda_events_list').isScrollable():
                self._device(resourceId= 'com.android.calendar:id/agenda_events_list', scrollable=True).scroll.vert.toBeginning()
            first_task = self._device(resourceId= 'com.android.calendar:id/agenda_item_text_container', instance = 0).child(resourceId= 'com.android.calendar:id/title').get_text()
            self._logger.debug("Delete task %s", first_task)  
            if self._device(resourceId= 'com.android.calendar:id/agenda_item_text_container').count > 1:
                second_task = self._device(resourceId= 'com.android.calendar:id/agenda_item_text_container', instance = 1).child(resourceId= 'com.android.calendar:id/title').get_text()
                self._logger.debug("The next task is %s", second_task)
            else:
                is_last = True
            self._device(resourceId= 'com.android.calendar:id/agenda_item_text_container', instance = 0).child(resourceId= 'com.android.calendar:id/title').click()
            self._device.delay(2)
        else:
            self._logger.debug("There is none task")
            return False
        if  self._device(resourceId= 'com.android.calendar:id/event_info_headline').child(resourceId= 'com.android.calendar:id/title').exists:      
            if self._device(resourceId= 'com.android.calendar:id/info_action_delete').exists:
                self._device(resourceId= 'com.android.calendar:id/info_action_delete').click()
                self._device.delay(2)
                
            if self._device(resourceId= 'com.android.calendar:id/delete').exists:
                self._device(resourceId= 'com.android.calendar:id/delete').click()
                self._device.delay(2)
            
            if self._device(resourceId= 'android:id/button1').exists:
                self._device(resourceId= 'android:id/button1').click()
                self._device.delay(2)
        if not self._device(resourceId = 'com.android.calendar:id/agenda_events_list').wait.exists(timeout=15000):
            self._logger.debug("Didn't back to event list.")
            return False
        if is_last:
            if not self._device(resourceId= 'com.android.calendar:id/agenda_item_text_container').exists:
            ...
            if self._device(resourceId= 'com.android.calendar:id/agenda_events_list').isScrollable():
                self._device(resourceId= 'com.android.calendar:id/agenda_events_list', scrollable=True).scroll.vert.toBeginning()
                self._device.delay(2)
            first_task_after = self._device(resourceId= 'com.android.calendar:id/agenda_item_text_container', instance = 0).child(resourceId= 'com.android.calendar:id/title').get_text()
