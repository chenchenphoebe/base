...
class Message(Common):
        ...
        if strtype == 'Photo':
            if self._device(resourceId='com.android.mms:id/image_view').exists:
                ...
                if self._device(resourceId='com.android.gallery3d:id/gl_root_view').exists \
                        ...
                        or self._device(resourceId='com.tct.gallery3d:id/photo_detail_root').exists \
            ...
            if self._device(resourceId='com.android.mms:id/play_slideshow_button').exists:
                ...
                self.choose_first_player()
                self.click_allow()
                if self._device(resourceId='com.android.mms:id/video').exists:
                    self._device(resourceId='com.android.mms:id/video').click()
                    self._device.delay(2)

                if self._device(resourceId='com.tct.gallery3d:id/button_restart', text='START OVER').exists:
                    self._device(resourceId='com.tct.gallery3d:id/button_restart', text='START OVER').click()
                    self._device.delay(3)

                if self._device(resourceId='com.tct.tablet.gallery3d:id/surface_view').exists:
                    if self._device(resourceId='com.tct.tablet.gallery3d:id/surface_view').wait.gone(timeout=45000):
                ...
                if self._device(resourceId='com.android.gallery3d:id/surface_view').exists:
                    ...
                    if self._device(resourceId='ccom.tct.gallery3d:id/surface_view').wait.gone(timeout=45000):
                        ...
                        resourceId='com.google.android.apps.photos:id/photos_videoplayer_button_holder').exists:
                    if not self._device(
                            resourceId='com.google.android.apps.photos:id/photos_videoplayer_button_holder').wait.gone(
                        timeout=20000):
                        self._device.press.back()
                    if self._device(resourceId='com.android.mms:id/play_slideshow_button').exists:
                        self._logger.debug('Open MMS video successfully.')
    ...
    def selcet_messaging_option(self, str_option, instance=0):
        ...
        while not self._device(resourceId='com.android.mms:id/forward').wait.exists(timeout=2000):
            self._device(resourceId='com.android.mms:id/mms_layout_view_parent').long_click()
            self._device.delay(2)
    ...
    def forward_messaging(self, sendNum, timeout, instance=0):
        ...
        if not self._device(resourceId='android:id/list').wait.exists(timeout=2000):
            self._device.swipe(526, 1835, 526, 422)
        if self._device(resourceId='com.android.mms:id/subject', textContains='Fwd:').exists:
            self._device(resourceId='com.android.mms:id/subject', textContains='Fwd:').click()
        elif self._device(resourceId='android:id/list').child(index=listitem - 5).exists:
        ...
        if self._device(resourceId='com.android.mms:id/date_view', textContains='Just now').exists \
                or self._device(resourceId='com.android.mms:id/date_view', textContains=':').exists:
    ...
    def delete_messaging_thread(self, index=0):
        ...
        if self._device(resourceId='android:id/list').exists:
            self._device(resourceId='android:id/list').scroll.vert.toBeginning(steps=100)
            self._device.delay(2)
            thread_num = self._device(resourceId='android:id/list').getChildCount()
