...
# *****************************************************************************
# Title:
#             1.test camera low light
# Precondition:
#             1.NA
# Procedure:
#             1.open the camera and open low light mode/taking photo 10times
...
class CheckCamera(unittest.TestCase):
    ...
    def testTakePhotoLowLight(self):
        global suc_times
        times = int(dicttest_times.get('take_photo_low_light'.lower(), 0))
        if times:
            logger.debug("start take low light photo test")
            if m_camera.stay_in_camera() and m_camera.switch_back_front("back") and m_camera.open_low_loght("on"):
                for loop in range(times):
                    try:
                        logger.debug('test taking low light photo ' + str(loop + 1) + ' Times')
                        if m_camera.take_photo("PHOTO", "3"):
                            suc_times += 1
            ...
            if m_camera.gallery_pic_del():
                ...
                m_camera.save_fail_img()
            logger.debug("----------------------test take low light photo completely")


if __name__ == "__main__":
    common.common.runTest(CheckCamera, [
        "testTakePhotoLowLight",
    ])
