...
#             1.NA
# Procedure:
#             1.open the camera for front and take a photo for 500 times
...
class CheckCamera(unittest.TestCase):
    ...
    def testTakePhotoFront(self):
        global suc_times
        times = int(dicttest_times.get('take_photo_front'.lower(), 0))
        if times:
            ...
            if times:
                ...
                if m_camera.gallery_pic_del():
                    ...
                    m_camera.save_fail_img()
                logger.debug("----------------------test take photo for front completely")
