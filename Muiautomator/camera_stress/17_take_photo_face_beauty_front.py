...
#             1.NA
# Procedure:
#             1.open the face beauty mode camera for front and take a photo for 500 times
...
class CheckCamera(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        m_com.clear_recent()
        logger.info("------------------Start check face beauty camera for front")
    ...
    def testTakePhotoFront(self):
        global suc_times
        times = int(dicttest_times.get('take_photo_face_beauty_front'.lower(), 0))
        if times:
            logger.debug("start take face beauty photo front test")
            if times:
                logger.debug("start take photo front test")
                if m_camera.stay_in_camera() and m_camera.switch_back_front("front"):
                    for loop in range(times):
                        try:
                            logger.debug('test tack photo for front ' + str(loop + 1) + ' Times')
                            if m_camera.take_photo("FACE BEAUTY", "2"):
                                suc_times += 1
                ...
                if m_camera.gallery_pic_del():
                    ...
                    m_camera.save_fail_img()
                logger.debug("----------------------test take face beauty photo for front completely")
