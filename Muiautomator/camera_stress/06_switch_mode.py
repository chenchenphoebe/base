...
# *****************************************************************************
# Title:
#             1.test camera for switching stereo
# Precondition:
#             1.NA
# Procedure:
#             1.open the camera and switch each take photo mode
...
class CheckCamera(unittest.TestCase):
    ...
    def testSwitchMode(self):
        global suc_times
        times = int(dicttest_times.get('switch_mode'.lower(), 0))
        if times:
            logger.debug("start switching mode test")
            if m_camera.stay_in_camera():
                for loop in range(times):
                    try:
                        logger.debug('test switching mode ' + str(loop + 1) + ' Times')
                        if m_camera.switch_mode():
                            suc_times += 1
                ...
                m_camera.save_fail_img()
            m_com.clear_recent()
            logger.debug("----------------------test switching mode completely")
