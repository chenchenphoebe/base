...
#             1.NA
# Procedure:
#             1.open the face beauty camera for back and take a photo for 500 times
...
class CheckCamera(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        m_com.clear_recent()
        logger.info("------------------Start check face beauty camera for back")
    ...
    def testTakePhotoBack(self):
        global suc_times
        times = int(dicttest_times.get('take_photo_face_beauty_back'.lower(), 0))
        if times:
            logger.debug("start take face beauty photo back test")
            if m_camera.stay_in_camera() and m_camera.switch_back_front("back"):
                for loop in range(times):
                    try:
                        logger.debug('test tack face beauty photo for back ' + str(loop + 1) + ' Times')
            ...
            if m_camera.gallery_pic_del():
                ...
                m_camera.save_fail_img()
            logger.debug("----------------------test take face beauty photo for back completely")
