...
# *****************************************************************************
# Title:
#             1.test camera filter for front
# Precondition:
#             1.NA
# Procedure:
#             1.open the camera filter for front and each filter take photo for 10 times
...
class CheckCamera(unittest.TestCase):
    ...
    def testCameraFilterFront(self):
        global suc_times
        times = int(dicttest_times.get('camera_filter_front'.lower(), 0))
        if times:
            logger.debug("start camera filter for front test")
            if m_camera.stay_in_camera() and m_camera.switch_back_front("front"):
                for loop in range(times):
                    try:
                        logger.debug('test camera filter for front ' + str(loop + 1) + ' Times')
                        if m_camera.open_filter():
                            suc_times += 1
            ...
            if m_camera.gallery_pic_del():
                ...
                m_camera.save_fail_img()
            logger.debug("----------------------test camera filter for front completely")


if __name__ == "__main__":
    common.common.runTest(CheckCamera, [
        "testCameraFilterFront",
    ])
