...
# *****************************************************************************
# Title:
#             1.test take photo mode
# Precondition:
#             1.NA
# Procedure:
#             1.open the camera,traverse each mode and take a photo
...
class CheckCamera(unittest.TestCase):
    ...
    def testTakePhotoMode(self):
        global suc_times
        times = int(dicttest_times.get('camera_mode'.lower(), 0))
        if times:
            logger.debug("start camera mode test")
            if m_camera.stay_in_camera() and m_camera.switch_back_front("back"):
                for loop in range(times):
                    try:
                        logger.debug('test camera mode ' + str(loop + 1) + ' Times')
                        if m_camera.switch_camera_mode():
                            suc_times += 1
            ...
            if m_camera.gallery_pic_del():
                ...
                m_camera.save_fail_img()
            logger.debug("----------------------test camera mode completely")
