...
# *****************************************************************************
# Title:
#             1.test camera video for back
# Precondition:
#             1.NA
# Procedure:
#             1.open the camera video for back and take a video for 3 min
...
class CheckCamera(unittest.TestCase):
    ...
    def testTakeVideoBack(self):
        global suc_times
        times = int(dicttest_times.get('take_video_back'.lower(), 0))
        if times:
            logger.debug("start take video back test")
            if m_camera.stay_in_camera() and m_camera.switch_back_front("back"):
                for loop in range(times):
                    try:
                        logger.debug('test take video for back ' + str(loop + 1) + ' Times')
            ...
            if m_camera.gallery_pic_del():
                ...
                m_camera.save_fail_img()
            logger.debug("----------------------test take video for back completely")
