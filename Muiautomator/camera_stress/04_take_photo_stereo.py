...
#             1.NA
# Procedure:
#             1.open the stereo mode and take a photo for 250 times
#             2.check the camera number
# Expectation:
#             1.take stereo photo successfully
# Platform:     7.0
# Project:      Pop5-6 4G
# author:       bei.han
# *****************************************************************************
...
class CheckCamera(unittest.TestCase):
    ...
    def testTakePhotoStereo(self):
        global suc_times
        times = int(dicttest_times.get('take_photo_stereo'.lower(), 0))
        if times:
            logger.debug("start take photo stereo test")
            if m_camera.stay_in_camera() and m_camera.switch_back_front("back"):
                for loop in range(times):
                    try:
                        logger.debug('test tack photo for stereo ' + str(loop + 1) + ' Times')
                        if m_camera.take_photo("BOKEH", '4'):
                            suc_times += 1
            ...
            if m_camera.gallery_pic_del():
                ...
                m_camera.save_fail_img()
            logger.debug("----------------------test take photo for stereos completely")
