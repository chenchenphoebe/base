...
# *****************************************************************************
# Title:
#             1.test camera filter for back
# Precondition:
#             1.NA
# Procedure:
#             1.open the camera filter for back and each filter take photo for 10 times
...
class CheckCamera(unittest.TestCase):
    ...
    def testCameraFilterBack(self):
        global suc_times
        times = int(dicttest_times.get('camera_filter_back'.lower(), 0))
        if times:
            logger.debug("start camera filter for back test")
            if m_camera.stay_in_camera() and m_camera.switch_back_front("back"):
                for loop in range(times):
                    try:
                        logger.debug('test camera filter for back ' + str(loop + 1) + ' Times')
            ...
            if m_camera.gallery_pic_del():
                ...
                m_camera.save_fail_img()
            logger.debug("----------------------test camera filter for back completely")


if __name__ == "__main__":
    common.common.runTest(CheckCamera, [
        "testCameraFilterBack",
    ])
