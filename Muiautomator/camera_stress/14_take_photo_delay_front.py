...
# *****************************************************************************
# Title:
#             1.test camera delay for front
# Precondition:
#             1.NA
# Procedure:
#             1.open the camera delay for front and cycle 10 times
...
class CheckCamera(unittest.TestCase):
    ...
    def testCameradelayFront(self):
        global suc_times
        times = int(dicttest_times.get('camera_delay_front'.lower(), 0))
        if times:
            logger.debug("start camera delay for front test")
            if m_camera.stay_in_camera() and m_camera.switch_back_front("front"):
                for loop in range(times):
                    try:
                        logger.debug('test camera delay for front ' + str(loop + 1) + ' Times')
            ...
            if m_camera.gallery_pic_del():
                ...
                m_camera.save_fail_img()
            logger.debug("----------------------test camera delay for front completely")


if __name__ == "__main__":
    common.common.runTest(CheckCamera, [
        "testCameradelayFront",
    ])
