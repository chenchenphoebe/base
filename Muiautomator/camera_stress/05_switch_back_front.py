...
# *****************************************************************************
# Title:
#             1.test camera for switching back and font
# Precondition:
#             1.NA
# Procedure:
#             1.open the camera and switch the font and back icon
#             2.check the whether the switch successfully
# Expectation:
#             1.switch the back and font successfully
# Platform:     7.0
# Project:      Pop5-6 4G
# author:       bei.han
# *****************************************************************************
...
class CheckCamera(unittest.TestCase):
    ...
    def testSwitchFontAndBack(self):
        global suc_times
        times = int(dicttest_times.get('switch_back_front'.lower(), 0))
        if times:
            logger.debug("start switching back and front test")
            if m_camera.stay_in_camera():
                for loop in range(times):
                    try:
                        logger.debug('test switching back and front ' + str(loop + 1) + ' Times')
                        if m_camera.switch_camera_back_front():
                            suc_times += 1
                ...
                m_camera.save_fail_img()
            m_com.clear_recent()
            logger.debug("----------------------test switching back and front completely")


if __name__ == "__main__":
    common.common.runTest(CheckCamera, [
        "testSwitchFontAndBack",
    ])
